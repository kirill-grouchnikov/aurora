        ...
        Column(modifier = Modifier.fillMaxSize().padding(top = 12.dp)) {
            for (i in 0..20) {
                Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.Bottom) {
                    val chroma = i * 4.0
                    LabelProjection(
                        contentModel = LabelContentModel(text = "Chroma=$chroma"),
                        presentationModel = LabelPresentationModel(
                            contentPadding = PaddingValues(0.dp),
                            horizontalAlignment = HorizontalAlignment.Trailing,
                            textMaxLines = 1,
                            singleLineDisplayPrototype = "Chroma=80.0"
                        )
                    ).project(modifier = Modifier.padding(start = 20.dp, end = 10.dp, bottom = 4.dp))
                    TonalPalettePreview(
                        tonalPalette = TonalPalette.fromHct(Hct.from(340.0, chroma, 40.0)),
                        showLegend = (i == 0)
                    )
                }
